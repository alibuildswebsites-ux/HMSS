...
const Header: React.FC<HeaderProps> = ({ userName }) => {
  ...
  const displayUser = userName || user?.name || 'Guest';

  if (!user) return null; // Only show dashboard header if logged in
    ...
    <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-4 sm:px-6 sticky top-0 z-30 w-full">
      ...
      <div className="flex items-center gap-3 sm:gap-6">
        <div className="hidden md:flex items-center text-sm text-gray-500 gap-2">
            <span>Good morning! <strong>{displayUser}</strong></span>
        </div>
        ...
        <div className="w-10 h-10 rounded-full bg-gray-200 overflow-hidden border-2 border-white shadow-sm shrink-0 flex items-center justify-center">
