...
const Dashboard: React.FC = () => {
  ...
  // Calculate width for occupancy bar (simple mock calculation)
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <StatsCard 
            ...
            mainVisual={
                <div className="h-16 w-full flex rounded-xl overflow-hidden mt-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-96">
        ...
        {/* Calendar Section (Static Placeholder as per design) */}
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
            ...
            <div className="flex gap-1 mb-4 overflow-x-auto pb-2 no-scrollbar">
                {['#1012', '#1013', '#1014', '#1022', '#1023'].map(room => (
                    <span key={room} className={`text-[10px] px-2 py-1 rounded border ${room === '#1014' ? 'border-gray-800 text-gray-900 font-bold' : 'border-gray-100 text-gray-400'}`}>{room}</span>
                ))}
            </div>
            ...
            <div className="space-y-3 flex-1 overflow-y-auto pr-1">
                {bookings.slice(0, 3).map((bk, i) => (
                    <div key={i} className={`flex items-center p-3 rounded-lg ${i % 2 === 0 ? 'bg-green-50/50' : 'bg-gray-50'}`}>
                        <div className="w-10 text-center mr-3 border-r border-gray-200 pr-3">
                            <span className="block text-xs font-bold text-gray-700">{13 + i}</span>
                            <span className="block text-[10px] text-gray-400">Wed</span>
                        </div>
                        <div className="flex-1">
                            <p className="text-xs font-medium text-gray-800">{bk.guestName}</p>
                        </div>
                    ...
                    {bookings.map((bk, i) => (
                        <div key={bk.id} className="flex items-center justify-between">
                            <div className="flex items-center gap-3">
                                <img src={`https://picsum.photos/40/40?random=${i+10}`} className="w-8 h-8 rounded-full" />
                                <div>
                                    <p className="text-sm font-medium text-gray-800">{bk.guestName}</p>
                                    <p className="text-xs text-gray-400">Room #{bk.roomNumber}</p>
                                </div>
                            </div>
                            <span className="text-xs text-gray-500">20/01/21 - 28/01/21</span>
                        </div>
                    ))}
                </div>
            </div>
